...
| **V1.10** | `versions/refix-v1.10.js` | p3.8 | Phase 2（2026-09-18）：F7 主动告警（`run-missing` / `host-method-error` 经 `agent/pre-step` 主动注入修复指引，同插件同症状每会话一次）+ 探针两级化（L1 `health` 调用面 → L2 结构面存活判定，消除无 health 插件的 10 轮盲区）+ 知识库导出/回填（`refix_export` / `refix_restore`）+ 同 Team 代修放行；**继承 V1.08 全部修复** | **稳定线当前推荐：动态包挂这个；要"一条命令装"见下方静态包 V1.4** |
| V1.08 | `versions/refix-v1.08.js` | p3.7 | 独立审查修复版（2026-09-18）：I-1 CONTRACT 补 `invoke` / I-2 report 走检测路径不吞 diff / I-3 自身身份反查回灌 U-7 / I-4 updater 跨会话校验 / I-5 cancelled 与 failed 分账 / I-7 去重键结构化 | 稳定线上一版（V1.3 静态包是它的形态） |
...
### 静态包（V1.4）—— 上表之外的另一种交付形态

上表列的全是**动态包**（源码 `.js`，由会话经 `cordis_define` 挂载）。从 **V1.2** 起，
同一个插件另有一条**静态包**发布线：当前产物是 `deploy/static/dist/dsh-refix-1.4.0.tgz`，
用宿主原生的 `dsh plugin --profile <p> add <tgz>` 安装，装完**随 dsh 启动自动加载**——
全程不需要模型参与、不需要贴挂载话术，也不需要 `tool-cordis`。

- 插件逻辑与 **V1.10（p3.8）逐字节相同**（852 行原样搬运、5 个 `harness.defineTool` 调用点）：V1.4 随源码修复一起前进。
- 版本记在 `versions/manifest.json` 的 `static` 段（顶层字段仍只描述动态包稳定线）。
- 构建、离线验证、真机验证、分发 → **[deploy/static/README.md](./deploy/static/README.md)**。

> ⚠️ **V1.1-pre 前瞻版本警示**：V1.1-pre 系列是面向 V1.1 正式版的**预览线，含实验性改动与潜在的破坏性变更**（详见下方"已知边界"中阶段 3 相关条目）。除非你需要体验更新提示 / 执行手册 / peer updater 功能，否则请使用 **V1.10**。慎重选择升级安装。
...
versions/        # 插件版本源码（plain JS 函数体，经 cordis_define 挂载）
  refix-v1.01.js ~ refix-v1.10.js   # 稳定发布线（V1.10 当前推荐）
  refix-v1.1-pre1.js ~ pre3.js      # ⚠️ 前瞻版本 V1.1-pre（pre3 由 tools/gen-pre.mjs 生成，勿手改）
...
ac/              # 验收脚本（真实 cordis Context + DynamicCordisRunnerService，不 mock runner）
  ...
  n-series-p2.static-check.mjs  # Phase 2 结构断言（42 项：P0~P4 + manifest + 产物，含 6 条反向对照）
  v110-smoke.mjs       # V1.10 行为冒烟（5 场景：F7 全链路 / 探针两级 / 知识库导出回填 / 同队放行 / 注册面）
  pre3-smoke.mjs       # 生成线等价性（pre2 ⊆ pre3 超集矩阵 + 继承矩阵）
...
| 字段 | 含义 |
|------|------|
| `version` | 当前 `REFIX_VERSION`（V1.10 = `p3.8`） |
...
| 检查点 | 期望值（以 V1.10 为例） |
|---|---|
| dsh 控制台那行 | `dsh-refix p3.8 ready; contract OK ...` |
| `refix_report` 的 `version` | `p3.8` |
...
```powershell
Get-FileHash -Algorithm SHA256 "$env:USERPROFILE\refix-v1.10.js"
```
...
| 文件 | 定版号 | 字节 | sha256 |
|---|---|---|---|
| `refix-v1.10.js` | V1.10（推荐） | 49279 | `e1892c7622fa2899dfce187509cebf96f394b48f96a0f1be87c8e5becbc1d3bb` |
| `refix-v1.1-pre3.js` | V1.1-pre3 ⚠️ | 63902 | `c20f3c54692774548dc4fd4ffaeab08993dcf4985d70e80cc2d0acafdbd677fd` |
| `refix-v1.08.js` | V1.08（上一版） | 37013 | `0cf42e50428fef2e2dd1aebb189b7a62515eddfaa47f1eec2fb5896d217f4066` |
...
> 上表哈希均用 GitHub raw 实际下载物逐个复算，与仓库工作区文件逐字节一致（同时排除 CRLF 污染：raw 侧 0 个 `\r\n`）。复算时间：前四个于 2026-09-17；`refix-v1.08.js` 于 2026-09-18（V1.08 发布后复算，至今未变）；`refix-v1.10.js` 与 `refix-v1.1-pre3.js` 于 2026-09-18 推送后复算。

### 回退到旧版本

版本链完整保留，V1.01 → V1.10 全部可切。回退 = 换一个 packageId 激活：
...
| **`refix-v1.10.js`** | **V1.10** | **Phase 2 四项能力 + 继承 V1.08 六项修复（当前推荐）** |
| `refix-v1.08.js` | V1.08 | 独立审查六项修复 |
...
**坑 2：没有状态指示灯** —— 想知道活着没有：看 dsh 控制台那行 `dsh-refix p3.8 ready; contract OK...`，或喊一句 `refix_report`。
...
V1.10 全量复跑（2026-09-18，本体仓库内，**独立复跑而非沿用构建方自述**）：结构断言 `build-static --check` **OK**（源 sha256 `e1892c76…`、**852 行 verbatim**、5 个 `defineTool` 调用点）；离线冒烟 `test-static` **41/41**、`--fallback` **43/43**（不带参数与显式入口两条路径均已实测可跑）；Phase 2 结构断言 `ac/n-series-p2.static-check.mjs` **42/42**；行为冒烟 `ac/v110-smoke.mjs` **PASS**（F7 全链路 / 探针两级 / 知识库导出回填 / 同队放行 / 注册面 共 5 场景）；生成线等价 `ac/pre3-smoke.mjs` **PASS**（pre2 注册面 ⊆ pre3 + 继承矩阵）；`tools/gen-pre.mjs` 连跑两次 SHA256 一致（`c20f3c54…`，幂等且不扰动产物）；真机启动 `boot-test`（隔离 profile 装入 `dsh-refix-1.4.0.tgz`）**PASS**：`dsh-refix p3.8 ready; contract OK`、零降级、零树失败；历史 AC 十三个脚本连跑 **13/13 PASS**（`p0 / p1 / p2 / p3 / p3r / p3r2 / p3r3 / upd / upd2 / upd3 / ac52 / probe-pre-step / bench`）。发布物 `dsh-refix-1.4.0.tgz`（19601B）解包后 4 个文件与 `packages/dsh-refix/` 逐字节一致。
...
- **版本源只报稳定线（2026-09-17 起）**：`manifest.json` 的 `latest` 指向**稳定线**（当前 `p3.8` / `V1.10`，带 `channel: "stable"`）。pre 线（V1.1-pre1 / pre2 / pre3 / updater）**不进入自动提示**——安装推荐稳定版的用户不会再被催着升到 pre 版；要装 pre 必须由**用户明确指名文件**（"用 `versions/refix-v1.1-pre2.js`"），模型不得自行建议或把版本源改指 pre。
...
| V1.10 Phase 2 | F7 主动告警 + 探针两级化 + 知识库导出/回填 + 同 Team 代修放行（继承 V1.08 全部修复）+ N 系列 P2 42 项断言 | ✅ 验收通过 |
...
| P0 生成式定版 | `tools/gen-pre.mjs` 从 V1.10 单源生成 `refix-v1.1-pre3.js`（7 锚点全消费、幂等、残留锚点即失败） | ✅ 幂等实测 + pre3-smoke 通过 |
