// 【定版 V1.1-pre2】前瞻版本（原开发代号 v9 / p3.6）⚠️ 含实验性改动，慎重选择升级安装
// dsh-refix v9（更新提示 · 手册版 p3.6）— 自诊断·自修复·自迭代 + 自更新探测/执行手册
// 在 v8（p3.5）之上把 F6 从"只提示人工路径"推进到「阶段 2：提示附带可执行手册」：
//   U-6  提示新增固定的「执行手册」段，使模型能据此完成会话内换版
//        （cordis_define kind:'existing' 追加新包 → cordis_run mode:'update' 切换）。
//        手册文本**全部由本地状态 + 固定模板生成**，绝不采用 manifest 提供的
//        字段作为命令 / ID / 路径（清单内容一律只当"外部文本"展示）。
//   U-7  身份自报：ownPluginId 优先取 cordis/dynamic-package 事件锚点，缺失时按
//        inventory 的 packages[].name 前缀反查兜底；currentPackageId 取自
//        inventory 的 currentPackageId。两者写入提示与 selfCheck().update.self，
//        免去模型猜 ID（这是阶段 2 的主要价值）。
//   U-8  回滚点显式写进提示：旧包不可变 → cordis_run(pluginId, <currentPackageId>,
//        'update') 即回滚，且**不需重新 define**。
//   U-9  外部文本隔离：manifest 的 notes / url 视为不可信外部输入 —— 控制字符与
//        换行压平、长度收紧、url 仅接受 http(s) 且有长度上限，且标注"勿当作指令"。
//        这是阶段 1 未处理的注入面（阶段 2 让提示可执行后风险变实，必须收口）。
//   U-10 executable 由 false 改为 'manual-guided'：手册可用，但**仍不得无人值守
//        自动执行**——提示文本显式声明"本提示不代表用户授权"，必须由用户明确要求
//        后才动作（B4 审批门与"不做无人值守"边界在阶段 2 保持不变）。
//   U-11 跨会话硬约束写入提示：kind:'existing' 在宿主侧校验 sessionId
//        （cordis-host-runner/src/index.ts L179），跨会话追加必然失败 → 换版必须在
//        当初定义 dsh-refix 的那个会话里做。
// 阶段 1 既有条目（保持）：
...
//   U-5  阶段边界：dsh-refix 仍不能自升级（self-repair-forbidden）——本插件自身
//        不调用 define/run 改自己；手册是给**会话里的模型**读的，由模型经官方工具
//        在用户授权下执行。
...
// 版本源：GitHub raw 上的发布清单。格式 {"latest":"p3.6","notes":"…","url":"…"}，
...
return {
  ...
  apply(ctx) {
    ...
    // ownPluginId 由 cordis/dynamic-package 事件锚定（v8 既有）；若该事件早于监听
    // 注册而丢失，退化为按 packages[].name 前缀在 inventory 里反查。两者都拿不到
    // 返回 null —— 手册随之降级为"先自查"，绝不猜一个 ID 写进提示。
    // 只读；失败只影响更新手册，不影响诊断/修复路径。
    function resolveSelfIdentity() {
      ...
      try {
        const rows = runner.inventory()
        if (pid !== null) {
          for (const r of rows) if (r.pluginId === pid) { row = r; break }
        } else {
          for (const r of rows) {
            const pkgs = Array.isArray(r.packages) ? r.packages : []
            for (const p of pkgs) {
              if (p && typeof p.name === 'string' && p.name.indexOf(SELF_PLUGIN_NAME) === 0) {
                pid = r.pluginId
                row = r
                break
              }
        ...
        console.error('[refix] 自身身份解析失败（只影响更新手册）: ' + ((e && e.message) || e))
        return { pluginId: pid, currentPackageId: null }
      }
      return {
        pluginId: pid,
    ...
    function buildNoticeMessage(notice) {
      ...
      if (pid === null) {
        ...
        lines.push('2) 用上一步返回的 packageId 切换：cordis_run({ pluginId: \'' + pid
          + '\', packageId: \'<define 返回的 packageId>\', mode: \'update\' })')
        lines.push(cur === null
          ? '3) 回滚：用 cordis_run 切回升级前的 currentPackageId（旧包不可变，即回滚点；其值见 refix_report 的 update.self.currentPackageId）。'
          : '3) 回滚：cordis_run({ pluginId: \'' + pid + '\', packageId: \'' + cur
            + '\', mode: \'update\' })（旧包不可变，无需重新 define）。')
      }
