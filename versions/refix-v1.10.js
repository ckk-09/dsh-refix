...
// @refix-gen:constants（gen-pre 锚点：pre 线在此注入 F6 常量；稳定线保持原样）
...
return {
  ...
  apply(ctx) {
    ...
    // @refix-gen:state（gen-pre 锚点：pre 线在此注入 F6 状态变量）
    ...
    function selfCheck(limit) {
      return {
        ...
        // @refix-gen:selfcheck-update（gen-pre 锚点：pre 线替换为 update 字段）
      }
    }
    ...
    // @refix-gen:events（gen-pre 锚点：pre 线在此注入 pre-step 监听）
    // @refix-gen:interval（gen-pre 锚点：pre 线将下一行替换为巡检+探测共用 tick）
    ...
    // @refix-gen:body（gen-pre 锚点：pre 线在此注入 F6 函数与逻辑体）
    ...
    // @refix-gen:ready（gen-pre 锚点：pre 线将下一行就绪日志替换为带 F6 状态的版本）
